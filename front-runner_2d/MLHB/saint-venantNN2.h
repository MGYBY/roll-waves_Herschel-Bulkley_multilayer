...
//     double shear = (h>0? pow(pow((up.x[]-um.x[])/(h*(layer[l]*0.50+layer[l-1]*0.50)), 2.0)+pow((up.y[]-um.y[])/(h*(layer[l]*0.50+layer[l-1]*0.50)), 2.0), 0.50) : HUGE);
...
//      double shear = (h>0? pow(pow((up.x[]*2.0)/(h*layer[l]), 2.0)+pow((up.y[]*2.0)/(h*layer[l]), 2.0), 0.50) : HUGE);
//      nueq[l] = nu_eq(shear);
//    }
//    }
   ...
   for (l = 1; l < nl; l++) {
     ...
     vector up = ul[l] ;
     // press -= h*layer[l];
     double shear = (h>0? pow(pow((up.x[]-um.x[])/(h*layer[l]), 2.0)+pow((up.y[]-um.y[])/(h*layer[l]), 2.0), 0.50) : HUGE);
     nueq[l] =  nu_eq(shear);
  }
  nueq[0] = nueq[1];
